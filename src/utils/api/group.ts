...
export async function createPayAccount(
  ...
  const url = `${BASE_URL}${API_ENDPOINTS.ACCOUNT.CREATE_GROUP_PAY.replace(
    "{groupId}",
    groupId
  )}`;
  ...
  try {
    ...
    throw {
      code: "NETWORK_ERROR",
      message:
        "페이 계좌 개설 요청에 실패했습니다. 네트워크 연결을 확인해주세요.",
      path: `/accounts/group-pay/${groupId}`,
      timestamp: new Date().toISOString(),
      status: 0,
    } as ApiErrorWithStatus;
...
export async function getGroupStatus(
  ...
  const url = `${BASE_URL}${API_ENDPOINTS.GROUP.STATUS.replace(
    "{groupId}",
    groupId
  )}`;
  ...
  try {
    ...
    throw {
      code: "NETWORK_ERROR",
      message:
        "그룹 상태 확인 요청에 실패했습니다. 네트워크 연결을 확인해주세요.",
      path: `/groups/${groupId}/status`,
      timestamp: new Date().toISOString(),
      status: 0,
    } as ApiErrorWithStatus;
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw {
        code: `HTTP_${response.status}`,
        message:
          errorData.message || `그룹 참여에 실패했습니다. (${response.status})`,
        path: `/groups/invites/${code}/accept`,
        timestamp: new Date().toISOString(),
        status: response.status,
      } as ApiErrorWithStatus;
...
export async function getGroupMembers(
  ...
  const url = `${BASE_URL}${API_ENDPOINTS.GROUP.MEMBERS.replace(
    "{groupId}",
    groupId
  )}`;
  ...
  try {
    ...
    throw {
      code: "NETWORK_ERROR",
      message:
        "그룹 멤버 조회 요청에 실패했습니다. 네트워크 연결을 확인해주세요.",
      path: `/groups/${groupId}/members`,
      timestamp: new Date().toISOString(),
      status: 0,
    } as ApiErrorWithStatus;
