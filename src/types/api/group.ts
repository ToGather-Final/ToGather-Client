...
export interface GroupPortfolio {
  ...
  message: string;
  data: {
    totalInvested: number; // 그룹 총 투자금액 (원)
    totalValue: number; // 그룹 총 평가금액 (원)
    totalProfit: number; // 그룹 총 평가손익 (원)
    totalProfitRate: number; // 그룹 총 수익률 (%)
    totalHoldings: number; // 그룹 보유 종목 수
    topHoldings: GroupHolding[];
    totalCashBalance: number; // 그룹 전체 예수금 (원)
  };
  errorCode?: string;
}
